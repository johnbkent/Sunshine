...
public class WeatherProvider extends ContentProvider {
    ...
    public int update(
        ...
        switch (match){
            case WEATHER: {
                long count = db.update(WeatherContract.WeatherEntry.TABLE_NAME,values,selection,selectionArgs);
                if (count > 0)
            ...
            case LOCATION:{
                long count = db.update(WeatherContract.LocationEntry.TABLE_NAME,values,selection,selectionArgs);
                if (count>0)
                    updateCount=(int) count;
                else
                    throw new android.database.SQLException("Failed to update " +uri);
